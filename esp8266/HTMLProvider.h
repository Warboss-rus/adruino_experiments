static const char HTML_PAGE[] PROGMEM = "<!DOCTYPE html><html><link rel=\"icon\" href=\"data:,\"><script>%s</script><body><h1>ESP8266 Temp monitor</h1><p>Current temperature: <span id='temp'>%.2f</span>&degC<br>Current pressure: <span id='pressure'>%.0f</span> mmHg (<span id='pressurePa'>%d</span> Pa)<br>Current humidity: <span id='humidity'>%.2f</span>%<br>Timestamp: <span id='timestamp'></span></p><br>Display scale: <select id='historyScale' onchange='changeScale(this)'><option value='' disabled hidden>Custom</option><option value='-1' selected>Everything</option><option value='3600000'>Last Hour</option><option value='86400000'>Last Day</option><option value='2592000000'>Last Month</option><option value='31536000000'>Last Year</option></select><br><canvas id=\"historyGraph\" width=\"1000px\" height=\"400px\" style=\"border: 1px solid #000000;\" hidden></canvas></body></html>";
...
{
  char result[sizeof(HTML_PAGE) + sizeof(HTML_SCRIPT) + 40];
  float pressureMmHg = currentState.pressure / 133.3224f;
  snprintf(result, sizeof(result), HTML_PAGE, FPSTR(HTML_SCRIPT), currentState.temperature, pressureMmHg, currentState.pressure, currentState.humidity);
  return result;
}
